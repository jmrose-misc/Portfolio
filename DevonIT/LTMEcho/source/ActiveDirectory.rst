...
LTM supports Active Directory Integration, which will allow Active
...
1. Access the LTM Administrator Console and select "*Configure Active
...
2. Select *Join Domain* and authenticate with an Active Directory Administrator 
   account. Once this is complete, LTM will state that it has successfully
...
3. If necessary, create a new Security Group in Active Directory. Otherwise,
   ...
   require specific permissions when accessing LTM, these will be set in the
